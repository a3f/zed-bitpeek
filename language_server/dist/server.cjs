...
function parseBitIndex(value) {
  ...
  if (!Number.isSafeInteger(bit) || bit < 0 || bit > MAX_MACRO_BIT_INDEX) {
    return null;
  }
  return bit;
}
...
function parseNumberAtPosition(doc, position) {
  ...
  if (numericWord.match(/^(0[bB]['01]*[01]|0[bB][_01]*[01])$/)) {
  ...
  } else if (numericWord.match(/^(0[oO]?['0-7]*[0-7]|0[oO]?[_0-7]*[0-7])$/)) {
    literalKind = "octal";
    ...
    for (let i = 0; i < numericWord.length; i++) {
      ...
      const digit = ch >= "0" && ch <= "9" ? ch - "0" : ch >= "a" && ch <= "f" ? ch.charCodeAt(0) - 87 : ch.charCodeAt(0) - 55;
      num = num * 16 + digit;
      bigNum = bigNum * 16n + BigInt(digit);
    }
    hexs = numericWord.substring(2);
  } else if (numericWord.match(/^([0-9]|[1-9]['0-9]*[0-9]|[1-9][_0-9]*[0-9])$/)) {
    literalKind = "decimal";
...
function macroReplacementActions(uri, parsed) {
  if (!parsed || parsed.literalKind != "hex" || parsed.hasNegativePrefix || parsed.bigNum <= 0n) {
    return [];
  }
